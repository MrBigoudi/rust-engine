...
use ash::{util::read_spv, vk::{ShaderModule, ShaderModuleCreateInfo, ShaderStageFlags}, Device};
...
impl Shader {
    ...
    pub fn create(device: &Device, stage_flag: ShaderStageFlags, shader_path_from_shaders_dir: &str, shader_entry_point: Option<&str>) -> Result<Self, EngineError> {
        let crate_path = env!("CARGO_MANIFEST_DIR");
        let spv_path = crate_path.to_owned() + &Self::get_compiled_shader_path(shader_path_from_shaders_dir);
        // open the file. With cursor at the end
        let mut spv_file = match std::fs::File::open(spv_path.clone()){
            Ok(file) => file,
            Err(err) => {
                error!("Failed to open the vulkan shader `{:?}': {:?}", shader_path_from_shaders_dir, err);
        ...
        let spv_code = match read_spv(&mut spv_file){
            Ok(code) => code,
            Err(err) => {
                error!("Failed to read the vulkan shader `{:?}': {:?}", shader_path_from_shaders_dir, err);
                return Err(EngineError::InitializationFailed);
            }
        };
        ...
        let shader_module = unsafe { 
            match device.create_shader_module(&create_info, None){
                Ok(module) => module,
                Err(err) => {
                    error!("Failed to create a vulkan shader module for shader `{:?}': {:?}", shader_path_from_shaders_dir, err);
